...
pred_path = 'F:/srtp/$RTP/git_files/SRTP/result/DANCE_%c_%d.json' %(dance_type,index)
...
def draw_skeleton_number(cvs, frame):
    for j in range(show_bones):
        cv2.putText(cvs, str(j), (int(frame[j][0]), (CANVAS_SIZE[1] - int(frame[j][1]))), cv2.FONT_ITALIC, scale * 0.3,(0, 0, 255), 1)
